...
collider* create_collider(entity* _owner, glm::vec2 _size) {
...
void delete_collider(collider* _collider) {
    if (!collider_list.empty()) {
        for (auto it = collider_list.begin(); it != collider_list.end();) {
            if (it->get() == _collider) {
                printf(" - deleting collider located at %p\n", it->get());
...
bool collider_vs_collider(collider* _collider) {
    for (const auto& collider : collider_list) {
        if (collider->owner->id != _collider->owner->id) {
...
bool ray_vs_collider(entity* _owner, ray_data& _hit, glm::vec2 _origin, glm::vec2 _direction) {
    for (auto& collider : collider_list) {
        if (collider->owner->id != _owner->id) {
            _hit.normal = { 0,0 };
            _hit.point = { 0,0 };
